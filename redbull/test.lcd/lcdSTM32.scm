...
(library 
 ...
 (define (lcd-config)
   ...
   (config-pin giod 13 #b01 #b01) ; PD13 = TFT-light   = gpio out, open-drain, 10 MHz
   (pin-set giod 13) ; back-light off
   (config-pin gioe  2 #b01 #b01) ; PE2  = TFT-RST     = gpio out, open-drain, 10 MHz
   (config-pin gioe  3 #b00 #b11) ; PE3  = TFT Reg Sel = gpio out, push-pull,  50 MHz
   (pin-clear gioe 3) ; Reg Sel off
   (for-each
    (lambda (pin) (config-pin giod pin #b10 #b11))
    '(0 1 8 9 10 14 15 4 5 7)) ; D2,3,13-15,0,1, ~OE, ~WE, ~E1 = AF, push-pull, 50 MHz
   (for-each
    (lambda (pin) (config-pin gioe pin #b10 #b11))
    '(7 8 9 10 11 12 13 14 15)) ; D4-12 = AF, push-pull, 50 MHz
   (write #x085010 fsmc  #x08) ; BCR2  <- SRAM, 16-bit, extended mode
   (write #x000811 fsmc  #x0c) ; BTR2  <- read:  14ns addr set, dat hold, 112ns dat set
   (write #x000113 fsmc #x10c) ; BWTR2 <- write: 14ns addr set, dat hold,  42ns dat set
   (register-copy-bit fsmc #x08 0 1) ; BCR2 <- enable Bank1 CS2 (SRAM)
   (lcop))
 ...
 (define (lcd-rd reg)
   ...
   (u16w reg)
   (pin-set gioe 3)
   (let ((val (bitwise-and #xffff (read #x6000000 0))))
     (pin-clear gioe 3)
     val))
 ...
 (define (lcd-init)
   ...
   (wlcd #x07 #x0173)
   (cls)
   (pin-clear giod 13)) ; back-light on
